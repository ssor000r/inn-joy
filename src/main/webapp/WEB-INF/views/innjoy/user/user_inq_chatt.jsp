...
<head>
	...
	<script type="text/javascript">
	    ...
	    $(document).ready(function() {
	        wsOpen(); // Open WebSocket connection when the page loads
	    ...
	    function wsOpen() {
	        ws = new WebSocket("ws://" + location.host + "/chating/" + $("#room_id").val());
	        ...
	        ws.onmessage = function(event) {
	            ...
	            if (message.type === "message") {
	                ...
	                // Display the received message based on sender
	                if (sender === $("#se_user").val()) {
	                    chatBox.append("<p class='me'>나: " + msgContent + "</p>");
	                } else if (sender === $("#re_user").val()) {
	                    chatBox.append("<p class='others'>" + sender + ": " + msgContent + "</p>");
	                }
    ...
    <div id="container" class="container">
        ...
        <div id="chating" class="chating">
            <c:forEach items="${chatMessages}" var="message">
                <c:choose>
                    <c:when test="${message.sender eq se_user}">
                        <p class="me">나: ${message.message}</p>
                    </c:when>
                    <c:otherwise>
                        <p class="others">${ceo_name} 사장님: ${message.message}</p>
                    </c:otherwise>
                </c:choose>
            </c:forEach>
        </div>
        ...
        <div>
            <div class="inputTable">
                <div class="input-group">
                    <input type="text" class="form-control" placeholder="보내실 메세지를 입력하세요." id="chatting">
                    <div class="input-group-append">
                        <button class="btn btn-outline-secondary" onclick="send()" id="sendBtn">
